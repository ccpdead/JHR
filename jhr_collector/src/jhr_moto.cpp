...
float Jhr_moto::pulse_equivalent = 8.1f;
...
void Jhr_moto::setTurnMode(float radian)
{
    ...
    float angular;   
    if(radian > 0)angular = 0.15; else angular = -0.15;
...
void Jhr_moto::TurnMode_handle()
{
    ...
    posRadian_old = posRadian;
    if(a1 > (3.1415926))a1 -= (3.1415926*2);
    if(a1 < (-3.1415926))a1 += (3.1415926*2);
    turn_Radian_curr += a1;
...
void Jhr_moto::rcv_callback_1(const char*buff_r,int iLen)
{
    ...
    if(crc == (unsigned char)buff_r[11] && motoRcv->head == 0x9bc2){
        ...
        is_ask = true;
        /*  int i1 = posRadian/3.1416 * 180;
            std::cout <<iCnt <<"  "<< lDistance << "    "<< rDistance << "  x= " << (int)posX << " y="
